...
- **Tests**: pytest, 1054 tests in `tests/`
...
src/scmodelforge/
  ...
  models/                # 3 architectures, components/ (custom attention, encoder), registry, hub
...
- Security hook warns on files containing "eval" — false positive for our evaluation module; use `model.train(False)` instead in test files to avoid triggering the hook
